    ...
    with open(log_path, 'a', encoding='utf-8', newline='') as f:
        ...
        csv_writer.writerow(
            ['Meta_iter', 'Inner_loss', 'Outer_loss', 'res_norm_y', 'res_norm_x', 'total_time', 'hyper_time',
             'lower_time', 'omin', 'y new', 'y final', 'x'])
    ...
    if args.load :
        with open(log_path, 'r', encoding='utf-8', newline='') as fnew:
            ...
            for row in csv_reader:
                if str(row['omin']) == 'omin':
                    break
                omin_load.append(int(str(row['omin'])))
    ...
    for meta_iter in range(args.outer_loop):
        ...
        with higher.innerloop_ctx(model_y, y_opt, copy_initial_weights=False) as (fmodel, f_opt):
            ...
            for iter in range(args.inner_loop):
                ...
                f_opt.step(loss_inner) # follower update: y^{k+1} = y^k -lr * grad_y_k f with (stochastic) gradient
                fmodel.toy_y = lim(fmodel.toy_y,-2,2) # projection onto [-2,2]
                # Append the leader's objective function value for each step
                if not args.load:
                    with torch.no_grad():
        ...
        y_init_opt.step() # learning step -> z_t + lr * grad_z_t
        ...
        print("Meta_Iter{}, Outer_loss{}, Inner_loss{}, x{}, y{}, forward_time{}, backward_time{}"
              .format(meta_iter, model_x(y_final_log, x_log).detach().numpy(), model_y(x_log, y_final_log).detach().numpy(), x_log.data.detach().numpy(),
                      # model_x.toy_x.data,
        ...
        with open(log_path, 'a', encoding='utf-8', newline='') as f:
            ...
            csv_writer.writerow(
                [meta_iter, model_y(x_log, y_final_log).detach().numpy(),
                 ...
                 backward_time,
                 omin, y_new_log.numpy(), y_final_log.numpy(), model_x.toy_x.detach().numpy(),
